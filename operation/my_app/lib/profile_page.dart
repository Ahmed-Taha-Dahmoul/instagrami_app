...
class _ProfilePageState extends State<ProfilePage>
  ...
  String _getDisplayName() {
    ...
    String username =
        (userInfo!['username'] as String?)?.trim() ?? ""; // Fallback
  ...
  Future<void> _fetchAllData() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _dataFetched =
              true; // Mark that data has been successfully fetched at least once
        });
      }
    } catch (e) {
  ...
  Future<void> fetchUserInfo() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            "${AppConfig.baseUrl}user-info/"), // Use your API endpoint from config
        headers: {
  ...
  Future<void> fetchUserCredit() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            "${AppConfig.baseUrl}payment/user/credit/"), // Use your API endpoint from config
        headers: {
          "Authorization": "Bearer $token",
          "Content-Type": "application/json",
        },
      ...
      if (mounted) {
        if (response.statusCode == 200) {
          ...
          throw Exception(
              "Failed to fetch user credit: Status Code ${response.statusCode}");
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  String get formattedCredit {
    ...
    if (creditDouble == null)
      return '\$ N/A'; // Handle parsing error gracefully
  ...
  Future<void> _handleLogout() async {
                  ...
                  child: Text("Logout",
                      style: TextStyle(
                          color: const Color.fromARGB(
                              255, 255, 50, 35))) // Style logout button
                  ),
            ],
          ),
        ) ??
    ...
    if (confirmed && mounted) {
      // Check confirmation and if widget is still mounted
      ...
      try {
        ...
        Navigator.of(context, rootNavigator: true).pushNamedAndRemoveUntil(
          ...
          (Route<dynamic> route) =>
              false, // Predicate always returns false to remove all routes
        );
      } catch (e) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          // Use CustomScrollView for flexible layout with headers
          slivers: [
            // --- Custom Header Area ---
            SliverPadding(
                          ...
                          onPressed: isLoading
                              ...
                              : () {
                                  ...
                                  Navigator.push(
                                    ...
                                    if (result == true) {
                                      print(
                                          "Recharge page indicated success, refreshing profile...");
                                      _fetchAllData();
                                    }
                                  });
                                },
            ...
            SliverPadding(
              padding: const EdgeInsets.fromLTRB(
                  16.0, 0, 16.0, 16.0), // Padding around the list
              sliver: SliverList(
                delegate: SliverChildListDelegate(
                  [
                    ...
                    _buildOptionTile(
                      ...
                      iconBackgroundColor:
                          Colors.lightBlue[50]!, // Light blue accent
                      title: "History",
                      subtitle:
                          "Transaction history", // Subtitle remains the same
                      onTap: () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => RechargeCardsStatusPage()),
                        );
                      },
                    ),
                    ...
                    _buildOptionTile(
                      icon: Icons
                          .receipt_long_outlined, // Receipt icon for subscriptions
                      iconBackgroundColor:
                          Colors.green[50]!, // Light green accent
                    ...
                    _buildOptionTile(
                      ...
                      iconBackgroundColor:
                          Colors.orange[50]!, // Light orange accent
                      title: "Edit Profile",
                    ...
                    SizedBox(
                        height:
                            10), // Add a little space before logout for visual separation
                    _buildOptionTile(
                      icon: Icons.logout, // Logout icon
                      iconBackgroundColor:
                          Colors.red[50]!, // Light red background accent
                      ...
                      // ),
                      subtitle:
                          "Sign out of your account", // Descriptive subtitle
                      onTap: isLoading
                          ? () {}
                          : _handleLogout, // Call the existing logout handler, disable during load
                    ),
                    // --- END LOGOUT TILE ---
  ...
  Widget _buildShimmerContainer(
      ...
      bool isWhiteOnBlue = false}) {
    // ... implementation ...
    return Shimmer.fromColors(
      baseColor:
          isWhiteOnBlue ? Colors.white.withOpacity(0.7) : Colors.grey[300]!,
  ...
  Widget _buildOptionTile({
    ...
    assert(title != null || titleWidget != null,
        'Either title or titleWidget must be provided to _buildOptionTile');
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(icon,
                      color: Colors.black87.withOpacity(0.7), size: 22),
                ),
                SizedBox(width: 16),
                Expanded(
